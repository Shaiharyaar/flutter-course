...
void main() async {
  ...
  testWidgets("Question and answer options are shown from API", (tester) async {
    final interceptor = nock(baseUrl).get("/topics/1/questions")
    ...
    // Define the test key.
    const testKey = Key('first-quiz-test');
  ...
  testWidgets(
      "Verifying Option select feedback of quiz for correct option selection",
      (tester) async {
    final quizInterceptor = nock(baseUrl).get("/topics/1/questions")
      ..reply(200, quizResponse);
    final feedbackInterceptor = nock(baseUrl)
        .post("/topics/1/questions/1/answers", json.encode({'answer': '20'}))
      ..reply(200, correctSelectionFeedbackResponse);
    ...
    // Define the test key.
    const testKey = Key('second-quiz-test');
    ...
    final selectOptionBtn = find.text('20');
    await tester.tap(selectOptionBtn);
    await tester.pump();
    expect(feedbackInterceptor.isDone, true);
    expect(find.text("Your answer is correct"), findsOneWidget);
  });
